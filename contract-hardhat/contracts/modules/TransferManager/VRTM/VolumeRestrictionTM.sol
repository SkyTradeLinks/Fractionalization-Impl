...
contract VolumeRestrictionTM is VolumeRestrictionTMStorage, TransferManager {
        ...
        returns (Result, bytes32)
    {
        ...
        if (success == Result.INVALID)
            return (success, bytes32(uint256(uint160(address(this))) << 96));
        ...
        returns (Result, uint256, uint256, uint256, uint256, uint256, uint256, bool)
    {
        ...
        if (!paused && _from != address(0) && exemptions.exemptIndex[_from] == 0) {
            ...
            if ((individualRestrictions.individualRestriction[_from].endTime >= block.timestamp && individualRestrictions.individualRestriction[_from].startTime <= block.timestamp)
                || (individualRestrictions.individualDailyRestriction[_from].endTime >= block.timestamp && individualRestrictions.individualDailyRestriction[_from].startTime <= block.timestamp)) {
            ...
            } else if ((globalRestrictions.defaultRestriction.endTime >= block.timestamp && globalRestrictions.defaultRestriction.startTime <= block.timestamp)
                || (globalRestrictions.defaultDailyRestriction.endTime >= block.timestamp && globalRestrictions.defaultDailyRestriction.startTime <= block.timestamp)) {
    ...
    function changeExemptWalletList(address _wallet, bool _exempted) public withPerm(ADMIN) {
        ...
        if (_exempted) {
            ...
            exemptions.exemptAddresses.pop();
        ...
        withPerm(ADMIN)
    {
        ...
        _checkInputParams(_allowedTokens, startTime, _rollingPeriodInDays, _endTime, _restrictionType, block.timestamp, false);
    ...
    {
        uint256 startTime = _getValidStartTime(_startTime);
        _checkInputParams(_allowedTokens, startTime, 1, _endTime, _restrictionType, block.timestamp, false);
    ...
    {
        uint256 startTime = _getValidStartTime(_startTime);
        uint256 checkTime = (individualRestrictions.individualDailyRestriction[_holder].startTime <= block.timestamp ? individualRestrictions.individualDailyRestriction[_holder].startTime : block.timestamp);
        ...
        withPerm(ADMIN)
    {
        ...
        _checkInputParams(_allowedTokens, startTime, 1, _endTime, _restrictionType,
            (globalRestrictions.defaultDailyRestriction.startTime <= block.timestamp ? globalRestrictions.defaultDailyRestriction.startTime : block.timestamp),
            true
        );
        globalRestrictions.defaultDailyRestriction = VolumeRestriction(
            _allowedTokens,
    ...
    */
    function _restrictionCheck(
        ...
        if (_restriction.endTime >= block.timestamp && _restriction.startTime <= block.timestamp) {
            ...
            (sumOfLastPeriod, fromTimestamp, daysCovered) = _bucketCheck(
                ...
                _isDefault,
                fromTimestamp,
                BokkyPooBahsDateTimeLibrary.diffDays(fromTimestamp, block.timestamp),
    ...
    function _validAllowedAmount(
        ...
        returns (uint256)
    {
        if (block.timestamp > dailyRestriction.endTime || block.timestamp < dailyRestriction.startTime)
            return allowedAmount;
        else if (block.timestamp > restriction.endTime || block.timestamp < restriction.startTime)
            return allowedDailyAmount;
        else
            return allowedDailyAmount < allowedAmount ? allowedDailyAmount : allowedAmount;
    }
        ...
        returns(bool, uint256, uint256)
    {
        ...
        if ( block.timestamp <= _restriction.endTime && block.timestamp >= _restriction.startTime) {
            ...
            if (_dailyLastTradedDayTime == 0 || _dailyLastTradedDayTime < _restriction.startTime)
                ...
                _dailyLastTradedDayTime = _restriction.startTime + BokkyPooBahsDateTimeLibrary.diffDays(_restriction.startTime, block.timestamp) * 1 days;
            else if (block.timestamp - _dailyLastTradedDayTime >= 1 days)
                _dailyLastTradedDayTime = _dailyLastTradedDayTime + BokkyPooBahsDateTimeLibrary.diffDays(_dailyLastTradedDayTime, block.timestamp) * 1 days;
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        if (_diffDays >= _rollingPeriodInDays) {
            ...
            for (i = 0; i < _diffDays; i++) {
                ...
                if (counter >= _rollingPeriodInDays) {
                    ...
                    uint256 temp = _bucketDetails.daysCovered - (counter - _rollingPeriodInDays);
                    temp = _bucketDetails.lastTradedDayTime - (temp * 1 days);
                    if (isDefault)
                        sumOfLastPeriod = sumOfLastPeriod - (bucketData.defaultBucket[_from][temp]);
                    else
                        sumOfLastPeriod = sumOfLastPeriod - (bucketData.bucket[_from][temp]);
                }
        ...
        _fromTime = _fromTime + (_diffDays * 1 days);
        ...
        returns (uint256)
    {
        ...
        if (_restriction.typeOfRestriction == RestrictionType.Percentage) {
            _allowedAmount = (_restriction.allowedTokens - securityToken.totalSupply()) / uint256(10) ** 18;
        } else {
            _allowedAmount = _restriction.allowedTokens;
        }
        return _allowedAmount - _sumOfLastPeriod;
    }
        ...
        internal
    {
        ...
        details.lastTradedTimestamp = block.timestamp;

        if (_amount != 0) {
            if (_lastTradedDayTime !=0) {
                details.sumOfLastPeriod = _sumOfLastPeriod + _amount;
                // Increasing the total amount of the day by `_amount`
                if (isDefault)
                    bucketData.defaultBucket[_from][_lastTradedDayTime] = bucketData.defaultBucket[_from][_lastTradedDayTime] + _amount;
                else
                    bucketData.bucket[_from][_lastTradedDayTime] = bucketData.bucket[_from][_lastTradedDayTime] + _amount;
            }
            if ((_dailyLastTradedDayTime != _lastTradedDayTime) && _dailyLastTradedDayTime != 0 && block.timestamp <= _endTime) {
                // Increasing the total amount of the day by `_amount`
                if (isDefault)
                    bucketData.defaultBucket[_from][_dailyLastTradedDayTime] = bucketData.defaultBucket[_from][_dailyLastTradedDayTime] + _amount;
                else
                    bucketData.bucket[_from][_dailyLastTradedDayTime] = bucketData.bucket[_from][_dailyLastTradedDayTime] + _amount;
            }
        }
    }
     ...
     */
    function getTokensByPartition(bytes32 _partition, address _tokenHolder, uint256 _additionalBalance) external view override returns(uint256) {
        uint256 allowedAmountToTransact;
        uint256 fromTimestamp;
        uint256 dailyTime;
        ...
        if (_partition == LOCKED) {
            if (allowedAmountToTransact == 0 && fromTimestamp == 0 && dailyTime == 0)
                ...
                return (currentBalance - allowedAmountToTransact);
        }
    ...
    function getTotalTradedByUser(address _user, uint256 _at) external view returns(uint256) {
        return (bucketData.bucket[_user][_at] + bucketData.defaultBucket[_user][_at]);
    }
     ...
     * @return allAddresses List of the restricted addresses
     * @return allowedTokens List of the tokens allowed to the restricted addresses corresponds to restricted address
     * @return startTime List of the start time of the restriction corresponds to restricted address
     * @return rollingPeriodInDays List of the rolling period in days for a restriction corresponds to restricted address.
     * @return endTime List of the end time of the restriction corresponds to restricted address.
     * @return typeOfRestriction List of the type of restriction to validate the value of the `allowedTokens`
