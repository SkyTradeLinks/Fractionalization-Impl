import {
  ...
  UserDividend,
  UserTokenDividendAggregate as UserTokenDividendAggregateSchema,
  MonthlyDividendAggregate
...
export function handleDividendCreation(event: ERC20DividendDepositedEvent): void {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    const amount: BigInt = dividend.getValue0();
    ...
    userDividend.save();
    const aggId = user + "-" + token;
    let utsAgg = UserTokenDividendAggregateSchema.load(aggId);
    if (!utsAgg) {
      utsAgg = new UserTokenDividendAggregateSchema(aggId);
      utsAgg.user = user;
      utsAgg.token = token;
      utsAgg.totalEntitled = BigInt.fromI32(0);
      utsAgg.totalClaim = BigInt.fromI32(0);
    }
    utsAgg.totalEntitled = utsAgg.totalEntitled.plus(amount);
    utsAgg.save();
  }
  ...
  const divId      = token + "-" + year.toString() + "-" + monthIndex.toString();
  let divAgg = MonthlyDividendAggregate.load(divId);
  if (!divAgg) {
    divAgg = new MonthlyDividendAggregate(divId);
    divAgg.token = token;
    divAgg.year  = year;
    divAgg.month  = monthIndex;
    divAgg.totalDividends = BigInt.fromI32(0);
  }
  divAgg.totalDividends = divAgg.totalDividends.plus(event.params._amount);
  divAgg.save();
...
export function handleDividendClaim(event: ERC20DividendClaimedEvent): void {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    utsAgg.totalClaim = utsAgg.totalClaim.plus(amount);
    utsAgg.save();
  }
