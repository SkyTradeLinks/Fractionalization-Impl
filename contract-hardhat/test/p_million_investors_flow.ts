...
describe("Load test for million investor flow", function() {
    ...
    let POLY = 1;
    let DAI = 2;
    const usdRatePerToken = 5000000000000000000n;
    ...
    describe("ST-20 Token Flow with Yield Simulation", async () => {
        ...
        it("Should successfully attach the first STO module to the security token", async () => {
            ...
            _ratePerTier.push([usdRatePerToken, usdRatePerToken]); // [ 5 USD/Token, 5 USD/Token ]
    ...
    describe("Buy tokens using on-chain whitelist", async () => {
        ...
        it("Should whitelist and issue tokens to many investors", async () => {
            ...
            const batchInvestors = await readInvestorsFromCSV( 5, offset);
            ...
            console.log({merkleTree: merkleTree.dump()})
            ...
            for (let i = 0; i < values.length; i++) {
                ...
                const amountDAI = usdRatePerToken * rawAmount;
